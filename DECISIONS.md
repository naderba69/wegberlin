...
Versioned A1, A2, B1, and B2 registries author four noun anchors per lesson plus one verb-preposition-case frame per A1 lesson and two per A2/B1/B2 lesson, render them German-first with optional collapsed case tables, and validate every record during `prebuild` through a level-driven contract that also allows Genitiv prepositions. This is a staged linguistic layer: A1–B2 now hold 336 nouns/262 frames across 84/84 published lessons (144 authored plus 118 derived from a valency inventory that measured 141 targets and closed every gap in the lesson texts), while exhaustive target-vocabulary coverage and human German review remain partial. See `docs/adr/ADR-010-progressive-lexical-grammar-layer.md`.
