...
Versioned A1, A2, B1, and B2 registries author four noun anchors per lesson plus one verb-preposition-case frame per A1 lesson and two per A2/B1/B2 lesson, render them German-first with optional collapsed case tables, and validate every record during `prebuild` through a level-driven contract that also allows Genitiv prepositions. This is a staged linguistic layer: A1–B2 now hold 558 noun records (336 authored anchors plus 222 derived from a glossary inventory that measured 280 target nouns and closed every gap) and 262 frames (144 authored plus 118 derived from a valency inventory that measured 141 targets and closed every gap), while vocabulary outside the reading glossaries and human German review remain partial. See `docs/adr/ADR-010-progressive-lexical-grammar-layer.md`.
