...
Versioned A1 and A2 registries author four noun anchors per lesson plus one verb-preposition-case frame per A1 lesson and two per A2 lesson, render them German-first with optional collapsed case tables, and validate every record during `prebuild` through a level-driven contract. This is a staged linguistic layer: A1–A2 now hold 192 nouns/72 frames across 48/48 lessons, while B1–B2 and exhaustive target-vocabulary coverage remain partial and no human German review has happened. See `docs/adr/ADR-010-progressive-lexical-grammar-layer.md`.
