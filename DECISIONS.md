...
A versioned A1 registry authors four noun anchors and one verb-preposition-case frame per lesson, renders them German-first with optional collapsed case tables, and validates every record during `prebuild`. This is a staged linguistic layer: A1 has 96 nouns/24 frames, while A2–B2 and exhaustive target-vocabulary coverage remain partial. See `docs/adr/ADR-010-progressive-lexical-grammar-layer.md`.
