...
## استكمال 2026-09-19 (v135): المصالحة من حالة الكاش عند ضياع رسالة الاكتمال

القرار: لا تُعتَبَر رسالة `DWNB_OFFLINE_PACK_DOWNLOAD` مصدر الحقيقة الوحيد لاكتمال التثبيت.
`public/sw.js` يبتلع فشل `postMessage` في `replyTo()` عن قصد لأن الصفحة قد تُغلق أثناء
التثبيت، فحين تضاع الرسالة تبقى البطاقة على «بدأ تنزيل الحزمة» رغم أن الحزمة مُروَّجة.
لذلك تلتزم اللوحة أثناء `busy` باستعلام `DWNB_OFFLINE_PACK_STATUS` كل 4 ثوانٍ، وتُنهي
الحالة فقط عندما يقبل `packInstallFinished` اللقطة (نفس الحزمة المطلوبة، نفس خيار الصوت،
`routeCount` موجب، ولقطة مختلفة عمّا قبل الضغط). لا يُحدَّث المصحِّح ولا العتبات ولا يُخفَّف
أي فحص حجم: القبول مشروط بدليل مكتوب في الكاش لا بتقدير زمني.

السجل الكامل والقياس في `docs/OFFLINE-PACK-INSTALL-FLAKE-2026-09-19.md` و
`docs/run-logs/2026-09-19-pack-install-reconcile/`.
