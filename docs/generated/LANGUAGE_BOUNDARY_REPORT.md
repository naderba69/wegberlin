...
Content SHA-256: `08c79b6de90fc298e147aced26f58c273964b86404151f290c7ce06f74d55cfe`

## Result

`PASS` — 180 TSX files and 7046 opening JSX tags were scanned.

| Contract | Count |
|---|---:|
| Explicit German fragments (lang=de + dir=ltr) | 406 |
| Explicit Arabic fragments (lang=ar + dir=rtl) | 4 |
| LTR tags audited | 445 |
| RTL tags audited | 4 |
| Auto-direction adaptive fragments | 2 |
| Technical/numeric/secret bidi scopes | 45 |
...
| src/components/offline-pack-control.tsx:350 | مقارنة Metadata فقط؛ ليست Diff دلالية للمحتوى ولا تثبت الحزمة تلقائيًا. | Arabic host + plaintext boundary |
...
| src/components/planning-preferences-control.tsx:20 | لا رفع تلقائي: «مكثفة» لا تطبق إلا بعد اختيارك. Check-in والطاقة و«لدي وقت أقل» تستطيع دائمًا خفضها. | Arabic host + plaintext boundary |
...
| src/components/settings-view.tsx:211 | تقدمك في IndexedDB. مفتاح AI أو عنوان Ollama يبقى في Session Storage ويُستبعد من النسخ الاحتياطية. | Arabic host + plaintext boundary |
| src/components/settings-view.tsx:229 | لوحة واحدة للمكتمل والناقص والفاشل في المحتوى والصوت وقدرات Runtime الحالية. | Arabic host + plaintext boundary |
| src/components/settings-view.tsx:232 | ملف محمول قابل للتحقق بـSHA-256. | Arabic host + plaintext boundary |
| src/components/settings-view.tsx:234 | التسجيلات الصوتية محفوظة في Media Store منفصل. | Arabic host + plaintext boundary |
| src/components/settings-view.tsx:237 | تصدير .dwnb | Arabic host + plaintext boundary |
| src/components/settings-view.tsx:238 | يشمل التقدم والخطة والأخطاء والكتابة وسجل المرشد المنظم، ويمكن تضمين التسجيلات وتشفير الحمولة بـAES-GCM. عبارة | Arabic host + plaintext boundary |
| src/components/settings-view.tsx:239 | دعم نسخ DWNB القديمة | Arabic host + plaintext boundary |
| src/components/settings-view.tsx:239 | v1 قديم ومدعوم للاستيراد حتى 2027-03-31 مع تحذير واضح. v2 غير مشفر وv3 مشفر صيغتان حاليتان، ومضمون دعمهما على  | Arabic host + plaintext boundary |
| src/components/settings-view.tsx:246 | عنوان Ollama المحلي | Arabic host + plaintext boundary |
| src/components/settings-view.tsx:246 | مفتاح API | Arabic host + plaintext boundary |
| src/components/settings-view.tsx:249 | طلب Descriptor واحد بموافقتك، بلا محتوى تعليمي. unknown تعني أن الوصف لم يصرّح، ولا نخمّن من الاسم. | Arabic host + plaintext boundary |
| src/components/settings-view.tsx:251 | يسأل المرشد موافقتك قبل كل نص يُرسل إلى Gemini أو OpenRouter أو Ollama. Gemini مقيد بالموديلات المتحققة، وOpen | Arabic host + plaintext boundary |
| src/components/settings-view.tsx:264 | المفتاح أو عنوان Ollama المؤقت | Arabic host + plaintext boundary |
...
| src/components/study-export-control.tsx:29 | تقويم ICS، تقرير أسبوعي قابل للطباعة/PDF، وبطاقات Anki TSV دون خادم. | Arabic host + plaintext boundary |
