...
`PASS` — 24 canonical grammar nodes with 31 prerequisite edges, 24/24 controlled mappings, 24/24 productive mappings, and 576 item-level reading/listening taxonomy rows passed strict validation.
...
Every one of 288 reading and 288 listening questions has exactly one function label. The first listening question in every lesson of the current curriculum inherits the existing authored gist-stage contract; other rows use explicit semantic signals, with specific fact questions defaulting to detail.
...
- all 576 source questions must be covered once;
