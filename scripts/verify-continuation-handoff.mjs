...
for (const text of ["104 implemented, 17 partial, 2 not implemented", "301/301", "29/29", "301 generated static/SSG pages", "298/298", "298/51/51/51/199"])
  requireText(status, text, "PROJECT_STATUS.md");
for (const text of ["301/301", "29/29"]) requireText(readme, text, "README.md");
...
if (academicAudit.schema?.counts?.totalRootObjects !== 3402 || academicAudit.schema?.schemaFamilies !== 12) fail("academic Zod counters drifted");
if (academicAudit.schema?.counts?.nounGrammarEntries !== 336 || academicAudit.schema?.counts?.verbPrepositionFrames !== 262) fail("A1-B2 lexical grammar counters drifted");
...
for (const text of ["3,402", "2,584", "348", "336/336"]) requireText(prompt, text, "continuation prompt academic audit");
...
for (const text of ["336 سجل اسم", "262 إطار فعل/حرف جر", "24/24 درس A1", "24/24 درس A2", "24/24 درس B1", "12/12 درس B2"]) requireText(prompt, text, "continuation prompt A1-B2 lexical grammar contract");
...
console.log(`- delivery: ${offline.routeCount} Offline routes, ${cacheName}, 301 unit/integrity and 29+29 browser tests documented`);
