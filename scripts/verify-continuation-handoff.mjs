...
if (partialIds.length !== 11) fail(`P0 partial table has ${partialIds.length} rows`);
...
for (const text of ["110 implemented, 11 partial, 2 not implemented", "350/350", "35/35", "301 generated static/SSG pages", "298/298", "298/51/51/51/199"])
  requireText(status, text, "PROJECT_STATUS.md");
for (const text of ["350/350", "35/35"]) requireText(readme, text, "README.md");
...
if (cacheName !== "dwnb-full-pack-v62") fail(`unexpected current cache ${cacheName}`);
...
requireText(e2e, 'caches.open("dwnb-level-pack-a1-v62")', "Playwright level pack cache contract");
...
requireText(read("src/core/coach/weekly-plan.ts"), "GRACE_DAYS_PER_WEEK", "P0-266 grace-day allowance");
requireText(read("src/core/coach/weekly-plan.ts"), 'status:"grace"', "P0-266 grace day status");
...
console.log(`- delivery: ${offline.routeCount} Offline routes, ${cacheName}, 350 unit/integrity and 35+35 browser tests documented`);
