...
export default function ReviewPage() {
  ...
  return <div className="focus-page">
    ...
    {card && reviewState ? <div className="flashcard-zone">
      <div className="review-card-meta"><span>{card.tags[0]}</span><strong>{card.tags[1]}</strong><small>{personalErrorCard ? (queued.isNew ? "بطاقة علاج شخصية جديدة · بلا mastery" : `علاج شخصي مستحق: ${new Date(queued.dueAt).toLocaleDateString("ar-TN")} · بلا mastery`) : queued.isNew ? "بطاقة درس جديدة" : `كانت مستحقة: ${new Date(queued.dueAt).toLocaleDateString("ar-TN")}`}</small></div>
      <button className={flipped ? "flashcard flipped" : "flashcard"} aria-keyshortcuts="Space" onClick={() => setFlipped(!flipped)}>
