...
function SessionClock({ session }: { session: FullExamSession }) {
  ...
  // 0 على الخادم وفي أول رسم للعميل حتى لا يختلف أول رسم عن HTML؛ القيمة الحقيقية تُضبط في الأثر أدناه.
  const [now, setNow] = useState(0);
  // أساس حتمي قبل الترطيب: لحظة بدء الجلسة نفسها (جزء من الحالة المحفوظة)، لا ساعة الجهاز.
  const clockAt = now || Date.parse(session.startedAt);
  const status = continuousSessionEffectiveStatus(session, new Date(clockAt));
  const remaining = continuousSessionRemainingSeconds(session, new Date(clockAt));
