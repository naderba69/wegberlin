...
import { packInstallFinished, type PackInstallExpectation } from "@/core/offline/pack-reconciliation";
...
export function OfflinePackControl() {
  ...
  const installWatch = useRef<{ expectation: PackInstallExpectation; applied: boolean } | null>(null);

  // public/sw.js writes the pack metadata into the live cache before it replies, and its
  // reply is best-effort by design (replyTo() swallows a closed port). Without a second
  // look, one lost message would leave the card on «بدأ تنزيل الحزمة» forever although the
  // pack is already promoted, so while a download is running the panel also asks the
  // worker for the authoritative status and settles from it when the install shows up there.
  useEffect(() => {
    if (!busy) return;
    const timer = window.setInterval(() => {
      const watch = installWatch.current;
      if (!watch || watch.applied) return;
      void sendWorkerCommand("DWNB_OFFLINE_PACK_STATUS")
        .then((status) => {
          if (watch.applied || !packInstallFinished(status, watch.expectation)) return;
          watch.applied = true;
          setMetadata((current) => ({
            ...current,
            installed: true,
            ...
            packLabel: status.packLabel,
            routeCount: status.routeCount,
            assetCount: status.assetCount,
            entryCount: status.entryCount,
            includesAudio: status.includesAudio,
            audioEntryCount: status.audioEntryCount,
            byteSize: status.byteSize,
            compressedPageByteSize: status.compressedPageByteSize,
            sizeManifestFingerprint: status.sizeManifestFingerprint,
            rollbackAvailable: current.installed || current.rollbackAvailable,
          }));
          setProgress(null);
          setMessage(`اكتمل تثبيت ${status.packLabel ?? watch.expectation.packId}${watch.expectation.includeAudio ? " مع الصوت الاختياري" : " دون الصوت الاختياري"}.`);
          void refreshStorage();
        })
        .catch(() => {
          // Status unreachable: the download reply path still stands.
        });
    }, 4_000);
  ...
  async function downloadPack() {
    ...
    const watch = {
      expectation: {
        ...
        before: { packId: before.packId, completedAt: before.completedAt, entryCount: before.entryCount, routeCount: before.routeCount },
      },
      applied: false,
    };
    installWatch.current = watch;
    setBusy(true);
    ...
    try {
      ...
      // An install the status poll already proved must not be rewritten by a late timeout.
      if (!watch.applied) {
        setProgress(null);
        setMessage(error instanceof Error ? error.message : "فشل تنزيل الحزمة الكاملة.");
      }
    } finally {
      installWatch.current = null;
      setBusy(false);
    }
  }
  ...
  return <section className="settings-card offline-pack-card">
    ...
    <p className="offline-pack-copy">
      كل مستوى حزمة مستقلة مع صفحات المنصة المشتركة. حزمة B2 تشمل تدريبات Goethe وtelc المنفصلة، بينما الحزمة الكاملة تضم كل دروس A1–B2 المنشورة ومسارات المنصة كلها؛ والأعداد أدناه تُقرأ من بيانات الحزمة نفسها لا من نص ثابت. لا تشمل أي حزمة مفتاح AI أو التسجيلات الشخصية.
    </p>
