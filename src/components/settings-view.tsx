...
import { useDeviceValue, useMounted } from "./device-value";
...
export function SettingsView() {
  ...
  // مفتاح الجلسة يُقرأ بعد الترطيب لا فيه: قراءته في مُهيِّئ الحالة تجعل أول رسم للعميل يختلف عن
    // HTML الخادم متى كان التبويب يحمل قيمة، وهذا هو سبب عطل React hydration رقم 418 على /settings.
  const storedKey = useDeviceValue(() => sessionStorage.getItem("dwnb-ai-key") ?? "", "");
  const [draftKey, setDraftKey] = useState<string | null>(null);
  const key = draftKey ?? storedKey;
  const sessionChecked = useMounted();
