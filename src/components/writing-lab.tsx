  ...
  return <div className="lab-page">
    ...
    <div className="writing-layout">
      ...
      <aside className="writing-feedback">
        ...
        {reviewedAnalysis?<><div className="writing-dimensions">{reviewedAnalysis.dimensions.map((dimension)=><article key={dimension.key} className={dimension.passed?"passed":""}><header><span>{dimension.passed?<Check size={13}/>:"—"}</span><strong>{dimension.labelAr}</strong></header><p>{dimension.detailAr}</p>{dimension.evidenceQuote&&<blockquote lang="de" dir="ltr">{dimension.evidenceQuote}</blockquote>}</article>)}</div><div className="feedback-box"><strong>ملاحظات مرتبطة بجملتك</strong>{reviewedAnalysis.feedback.map((item)=><p key={item}>{item}</p>)}</div><WritingRepairPractice exercises={repairExercises} attempts={repairAttempts} onAttempt={recordRepairAttempt}/>{reviewedSubmission&&<HybridWritingReview sourceText={reviewedText} sourceSubmissionId={reviewedSubmission.id} taskId={taskId} sourceVersion={reviewedSubmission.version} level={level} taskPromptDe={task.promptDe} localPatternIds={reviewedAnalysis.errorPatterns.map((pattern)=>pattern.patternId)}/>}<details className="translation-panel" onToggle={(event)=>{if(event.currentTarget.open)recordModelReveal()}}><summary>النموذج بعد فحص المسودة</summary><p lang="de" dir="ltr">{task.modelDe}</p></details></>:<div className="feedback-placeholder"><FilePenLine size={26}/><p>أكمل التخطيط والمسودة والتحقق الذاتي. لن نستبدل نصك بإجابة مثالية صامتة.</p></div>}
      </aside>
    </div>
  </div>;
}
