...
export type GeminiWritingReviewAnswer={summaryAr:string;issues:WritingAIReviewIssue[];unresolvedAr:string[];provider:"gemini";model:string;promptVersion:typeof WRITING_REVIEW_PROMPT_VERSION};
...
const writingReviewPayloadSchema=z.object({summaryAr:z.string().trim().min(10).max(1200),issues:z.array(z.object({category:z.enum(["grammar","word-order","vocabulary","coherence","register","task-fulfillment","uncertain"]),excerpt:z.string().trim().min(1).max(180),explanationAr:z.string().trim().min(5).max(600),suggestionDe:z.string().trim().min(1).max(300),confidence:z.enum(["medium","high"])}).strict()).max(8),unresolvedAr:z.array(z.string().trim().min(3).max(300)).max(5)}).strict();
...
export function parseWritingReviewPayload(raw:string,sourceText:string){
  ...
  const source=sourceText.normalize("NFC").replace(/\s+/gu," ").trim().toLocaleLowerCase("de-DE");for(const issue of parsed.issues){const excerpt=issue.excerpt.normalize("NFC").replace(/\s+/gu," ").trim().toLocaleLowerCase("de-DE");if(!source.includes(excerpt))throw new Error("رفضنا ملاحظة Gemini لأنها لا تقتبس جزءًا فعليًا من النص الموافق على إرساله.")}
  return parsed;
}
...
function writingReviewSystemPrompt(context:WritingReviewRequestOptions["context"]){return[
  ...
  `Return ONLY JSON: {\"summaryAr\":string,\"issues\":[{\"category\":\"grammar|word-order|vocabulary|coherence|register|task-fulfillment|uncertain\",\"excerpt\":string,\"explanationAr\":string,\"suggestionDe\":string,\"confidence\":\"medium|high\"}],\"unresolvedAr\":string[]}.`,
...
export async function askGeminiWritingReview(config:AIConfig,learnerText:string,options:WritingReviewRequestOptions):Promise<GeminiWritingReviewAnswer>{
  ...
  const parsed=parseWritingReviewPayload(await responseJsonText(response,"gemini"),text);
