...
export const learningStateSchema = z.object({
  ...
  reviewReminderSettings:z.object({policyVersion:z.literal("local-review-reminder-v1"),enabled:z.boolean(),hourLocal:z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/),timeZone:z.string().min(1),lastDeviceNotificationDate:z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),dismissedInAppDate:z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),deliveryBoundary:z.literal("in-app-and-notification-api-while-open-no-push-background-guarantee-mastery-or-penalty")}).strict().default({policyVersion:"local-review-reminder-v1",enabled:false,hourLocal:"18:00",timeZone:"UTC",deliveryBoundary:"in-app-and-notification-api-while-open-no-push-background-guarantee-mastery-or-penalty"}),
  ...
  externalEvaluatorNotes: z
    .array(
      z.object({
        id: z.string().min(1),
        policyVersion: z.literal("external-evaluator-packet-v1"),
        submissionId: z.string().min(1),
        taskId: z.string().min(1),
        level: z.enum(["A1", "A2", "B1", "B2"]),
        reviewerLabel: z.string().min(1).max(80),
        receivedAt: z.string().min(1),
        notes: z.array(z.object({ excerpt: z.string().max(240), comment: z.string().min(1).max(400) })).max(24),
        unconfirmableExcerpts: z.array(z.string().max(240)).max(24),
        refusedPhrases: z.array(z.string().max(120)).max(24),
        boundary: z.object({
          source: z.literal("external-human-arranged-by-learner"),
          appVerifiedTheFeedback: z.literal(false),
          canCertifyExamReadiness: z.literal(false),
          canCountTowardLevelGate: z.literal(false),
          officialResultClaimedAr: z.string().min(1),
        }),
  ...
  speakingAttempts: z.array(z.object({
    ...
    transcriptConfirmation: z.object({policyVersion:z.literal("speaking-confirmed-transcript-v1"),source:z.enum(["local-asr-draft","typed-by-learner"]),confirmedText:z.string().max(1200),editedFromDraft:z.boolean(),confirmation:z.literal("learner-confirmed"),confirmedAt:z.string(),evidenceBoundary:z.literal("no-review-sent-before-learner-confirmation-no-audio-transmitted")}).strict().optional(),
