...
// Single source of truth: this string is persisted into review evidence, so two
// definitions could agree today and disagree tomorrow without any type error.
import { WRITING_REVIEW_PROMPT_VERSION } from "@/core/ai/client";

export { WRITING_REVIEW_PROMPT_VERSION };
...
export async function createWritingAIReviewEvidence(input:{sourceText:string;sourceSubmissionId:string;taskId:string;sourceVersion:number;model:string;summaryAr:string;issues:WritingAIReviewIssue[];unresolvedAr:string[];needsHumanReview?:boolean;now?:Date}):Promise<WritingAIReviewEvidence>{
