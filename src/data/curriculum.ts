...
const seeds: Record<CEFRLevel, ModuleSeed[]> = {
  ...
  B2: [
    ...
    { titleDe: "B2-Prüfungsreife", titleAr: "الجاهزية للامتحان", lessons: [["Schreiben und Sprechen", "الكتابة والمحادثة", "أنجز مهامًا إنتاجية تحت الوقت."], ["Generalprobe", "البروفة النهائية", "أثبت جاهزيتي بأدلة حديثة."], ["Die eigene Position revidieren", "مراجعة الموقف أمام الأدلة", "أقيّد موقفي بقدر دليله وأراجع نفسي."], ["Kohäsion im langen Text", "تماسك النص الطويل", "أمسك نصي بروابط واضحة وإحالات قاطعة."], ["Zeitplan unter Druck", "خطة الوقت تحت الضغط", "أُخطّط وقتي بمراحل واضحة وأعالج مصدر الخطأ."], ["Nachlese ohne Anklage", "مراجعة بلا اتهام", "أصوغ ما فاتني شرطًا وأبقي الانطباع ظنًا."], ["Forderungen ohne Täter: Passiversatzformen", "طلب بلا فاعل: بدائل المبني للمجهول", "أوزّع الإلزام والإمكان والتقييم في نصّي الرسمي."], ["Funktionsverbgefüge: Das Nomen steuert", "اسمٌ يقود وفعلٌ يُنفّذ: التراكيب الوظيفية في الكتابة الرسمية", "أُظهر مرحلة الإجراء بتركيب واحد وأحذف ما ينفخ النص."], ["Modalpartikeln: Wie ein kurzes Wort die Haltung sagt", "أدوات النبرة: كلمة قصيرة تحمل الموقف كلّه", "أختار الأداة بموقفها لا بترجمتها، وأحذفها من الطلب المؤرَّخ."], ["Zweiteilige Konnektoren: Ein Satz, zwei Klammern", "روابط ثنائية الأجزاء: قوسان يحدّدان الوزن", "أُظهِر العلاقة بين فكرتين بقوس تركيبي واحد وأتحكم في موضع الثقل."], ["Berichtete Rede: Wer spricht, und wer behauptet nur?", "النقل غير المباشر: من تكلّم فعلًا، ومن يدّعي فحسب؟", "أنقل خلافًا بمسافة بلا حكم."], ["Attribute, die den Satz strecken: Partizipien und Zusätze", "نعتٌ يمدّ الجملة: اسم الفاعل والمفعول والتوسعة", "أضغط الوصف وأفكّه عند الحاجة."], ["Ketten lösen: Genitiv und Kompositum", "سلاسل تُفكّ: إضافة الاسم والمركّب الاسمي", "أُعيد بناء السلسلة جملًا مفهومة."], ["Kommas, die Struktur zeigen: Zeichensetzung im langen Satz", "فواصل تُظهر البنية: الترقيم في الجملة الطويلة", "أُغلق أقواسي وأفحص نصي قبل التسليم."]]},
  ],
};
...
/**
 * Sequential per-level numbering with a cumulative counter, so a module that already has more
 * lessons than its siblings cannot shift the ids of later lessons (the previous formula reused the
 * current module's length for every module and broke as soon as module sizes diverged).
 */
export const publishedLessonLocalOrder: Record<CEFRLevel, number> = { A1: 24, A2: 24, B1: 24, B2: 24 };

export const curriculum = (Object.entries(seeds) as Array<[CEFRLevel, ModuleSeed[]]>).flatMap(
  ([level, modules]) => {
    let localOrder = 0;
    return modules.flatMap((module, moduleIndex) =>
      module.lessons.map(([titleDe, titleAr, objectiveAr]) => {
      localOrder += 1;
      const order = levelOffsets[level] + localOrder;
      return {
        ...
        status: localOrder <= publishedLessonLocalOrder[level] ? "published" : "planned",
