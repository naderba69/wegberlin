...
export interface WritingAIReviewIssue {
  ...
  /** German rule sentence the reviewer claims applies; required for `high`. */
  ruleDe?: string;
  /** Short drill the learner can do next, derived from this issue only. */
  remediationAr?: string;
  /** Reviewer's own doubt flag; absence is treated as "needs review". */
  needsHumanReview?: boolean;
}

export type ExternalEvaluatorEvidence = {
  id: string;
  ...
  receivedAt: string;
  notes: { excerpt: string; comment: string }[];
...
export interface WritingAIReviewEvidence {
  ...
  /** Set by the grounding gate: false only when nothing was rejected or open. */
  needsHumanReview?: boolean;
  createdAt: string;
}
...
export interface SpeakingTranscriptConfirmation {
  ...
  confirmedAt: string;
  evidenceBoundary: "no-review-sent-before-learner-confirmation-no-audio-transmitted";
}
...
export interface SpeakingAttempt {
  ...
  transcriptConfirmation?: SpeakingTranscriptConfirmation;
...
export interface LearningState {
  ...
  /**
   * Statements from a human the learner arranged outside this app. Optional so
   * every record written before this bridge still parses, and never read by a
   * level gate: it is reviewable advice, not an assessment result.
   */
  externalEvaluatorNotes?: ExternalEvaluatorEvidence[];
