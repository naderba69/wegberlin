...
const PACK_CACHE_NAME = (await readFile("public/sw.js", "utf8")).match(/const PACK_CACHE = "([^"]+)"/)?.[1] ?? "dwnb-full-pack-v161";
...
test("library and exam hubs render their completed content contracts", async ({ page }) => {
  test.setTimeout(60_000); // measured: cold first content-heavy page after a fresh production server start, 30s default was hit once in a full-suite run and the same test passed isolated in 33s wall clock
  ...
  await expect(biographyExtras).toContainText("أسماء هدف إضافية موثقة · 21");
...
test("P0 writing lab enforces plan, draft, self-check, cited feedback, and revision", async ({ page }) => {
  ...
  const bridge=page.locator('[data-external-evaluator-policy="external-evaluator-packet-v1"]');
  await expect(bridge).toBeVisible();
  await bridge.getByLabel("معايير المهمة للمراجع الخارجي").fill("Aufgabe erfüllen\nKohärenz");
  const packetPreview=bridge.locator(".external-evaluator-packet");
  await expect(packetPreview).toContainText("Aufgabe erfüllen");
  await expect(packetPreview).toContainText("بلا ادعاء مطابقة فحص رسمي");
  await expect(packetPreview).toContainText("نصي:");
  await expect(packetPreview).toContainText("رصد المحلّل المحلي");
  await expect(packetPreview).toContainText("(A1 · schreiben)");
  await bridge.getByLabel("اسم المراجع أو صفته (يُحفظ كما كتبته)").fill("أ. هدى");
  await bridge.getByLabel("الصق رده (سطر لكل ملاحظة)").fill("- جملة السبب تحتاج weil مع الفعل في النهاية.\n- Ergebnis: bestanden.");
  await bridge.getByRole("button",{name:/احفظ الرد كرأي خارجي/}).click();
  await expect(bridge.locator(".external-evaluator-rows li").first()).toContainText("أ. هدى");
  await expect(bridge).toContainText("عبارات درجة أو نتيجة مرفوضة: 1");
  await expect(bridge).toContainText("لا يمنح هذا التطبيق قرار امتحان");
...
test("generated exam clips play with limits, provenance labels, and TTS fallback", async ({ page }) => {
  test.setTimeout(60_000); // measured: 29.8s wall clock isolated on this 1.9 GB machine (real MP3 playback plus manifest fetch), so the 30s default is marginal under full-suite load
...
test("the optional full content pack opens unvisited lessons and exam tasks offline", async ({ page, context }) => {
  ...
  const afterAudioRemoval = await page.evaluate(async (PACK_CACHE_NAME) => {
    const cache = await caches.open(PACK_CACHE_NAME);
...
test("representative 320–1920 px viewports avoid document-level horizontal overflow", async ({ page }) => {
  // 18 navigations with hydration in one test on a single 512 MB `next start` worker; the wall-clock
  // allowance is raised, no overflow assertion is relaxed or removed.
      ...
      for (const route of routes) {
        ...
        const audit = await page.evaluate(() => {
          const visible = (element: Element) => {
            ...
            // Chromium >=151 keeps the last-known size of children inside a closed <details>
            // (content-visibility: hidden), so display/visibility alone would flag unpainted
            // drawer text as "off-screen educational text". Requiring real paint state keeps the
            // assertion about rendered learner text without widening any tolerance.
            if (element.closest("details:not([open])")) return false;
            return style.display !== "none" && style.visibility !== "hidden" && style.contentVisibility !== "hidden" && rect.width > 0 && rect.height > 0;
          };
...
test("settings restores a session credential after hydration without a mismatch", async ({ page }) => {
  ...
  await page.goto("/settings");
  await waitForLearningReady(page);
  // العطل لم يظهر عند أول زيارة، بل عند إعادة تحميل والتبويب يحمل المفتاح: هذه هي الحالة بالضبط.
  await page.evaluate(() => sessionStorage.setItem("dwnb-ai-key", "http://127.0.0.1:11434"));
  await page.reload();
  await waitForLearningReady(page);

  const credentialCard = page.locator(".privacy-delete-list article").filter({ hasText: "المفتاح أو عنوان Ollama المؤقت" });
  await expect(credentialCard).toBeVisible();
  await expect(credentialCard).toContainText("موجود في Session Storage الآن");

  await page.getByLabel("المزوّد").selectOption("local");
  await expect(page.getByLabel("عنوان Ollama المحلي")).toHaveValue("http://127.0.0.1:11434");

  const hydrationNoise = consoleErrors.filter((line) =>
    /hydrat|did not match|#418|Text content did not match|Minified React error/i.test(line));
  expect(hydrationNoise).toEqual([]);

});

test("nursing layer reframes level language, fails guessing, and stays out of gates", async ({ page }) => {
  test.setTimeout(90_000); // heavy route: 14-stage lesson navigation plus an IndexedDB read on both projects
  // A1 unit: "توابل" reuses A1 material inside a care situation. The environment must fail guessing and
  ...
  await expect(unit).toHaveAttribute("data-nursing-disclaimer", "هذه صياغات لغوية لا إرشاد سريري");
  await expect(unit.locator('[data-nursing-unit-disclaimer="true"]')).toContainText("هذه صياغات لغوية لا إرشاد سريري");
  await expect(unit.locator('[data-nursing-review-words]')).toHaveCount(0);

  // Measure stored learning state BEFORE the nursing interaction, so the check is "nothing changed"
  // rather than "nothing exists" (the lesson page itself seeds its own mastery keys).
  const readState = () => page.evaluate(() => new Promise<{ mastery: Record<string, number>; attempts: number; errors: number }>((resolve, reject) => {
    const open = indexedDB.open("der-weg-nach-berlin", 4);
    open.onerror = () => reject(open.error);
    open.onsuccess = () => {
      const request = open.result.transaction("learning-state", "readonly").objectStore("learning-state").get("primary");
      request.onerror = () => reject(request.error);
      request.onsuccess = () => resolve({ mastery: request.result.mastery, attempts: request.result.exerciseAttempts.length, errors: request.result.errors.length });
    };
  }));
  const before = await readState();

  const guess = unit.locator('[data-nursing-option-kind="guess"]').first();
  await guess.click();
  await expect(guess).toHaveAttribute("data-nursing-verdict", "rejected");
  await expect(unit.locator('[data-nursing-outcome]')).toHaveAttribute("data-nursing-outcome", "rejected");

  const accepted = unit.locator('[data-nursing-option-kind="recognize"]');
  await accepted.click();
  await expect(accepted).toHaveAttribute("data-nursing-verdict", "accepted");
  await expect(unit.locator('[data-nursing-outcome]')).toHaveAttribute("data-nursing-outcome", "accepted");
  await expect(unit.locator('[data-nursing-outcome]')).toContainText("لا إتقان ولا دليل ولا بوابة مستوى");

  // The layer must not touch stored learning state: same mastery map, same attempt and error counts.
  const after = await readState();
  expect(after).toEqual(before);

  // B1 unit: "طبق ثانٍ" adds memorisable professional words and hands them to the existing TSV importer.
  ...
  await expect(b1Unit).toHaveAttribute("data-nursing-cluster", "questioning");
  await expect(b1Unit.locator('[data-nursing-review-words="2"]')).toContainText("die Übergabe");
  await expect(b1Panel).toContainText("German → Arabic → Example → Tags");
  await expect(b1Unit).toContainText("هذه صياغات لغوية لا إرشاد سريري");
});
