...
import { buildRetrievalWarmup, composeTodayMission, getCoachTarget } from "@/core/coach/coach";
...
describe("coach session composer", () => {
  ...
  it("adds the no-score warm-up for a returning absolute beginner only after vocabulary exposure",()=>{const state={...defaultState,lessonProgress:{"a1-01":2},profile:{name:"Beginner",targetExam:"goethe-b2" as const,dailyMinutes:30 as const,arabicSupport:"modern-standard-arabic" as const,currentLevel:"A1" as const,priorExperience:"none" as const,createdAt:"2026-09-02T00:00:00Z"}};const mission=composeTodayMission(state);expect(mission.map((item)=>item.kind)).toEqual(["check-in","warmup","lesson","reflection"]);expect(mission.reduce((sum,item)=>sum+item.minutes,0)).toBe(30)});
  ...
  it("builds a German-first three-item warm-up from the current target without creating cards",()=>{const state={...defaultState,diagnosticResult,completedLessonIds:[],dueReviews:0,profile:{name:"Test",targetExam:"goethe-b2" as const,dailyMinutes:20 as const,arabicSupport:"modern-standard-arabic" as const,currentLevel:"A1" as const,createdAt:"2026-08-26T00:00:00Z"}};const items=buildRetrievalWarmup(state,new Date(2026,8,5,9));expect(items).toHaveLength(3);expect(items[0]).toMatchObject({lessonId:"a1-01",answerDe:"Hallo!",cueAr:"مرحبًا!",policyVersion:"pre-srs-retrieval-warmup-v1"});expect(composeTodayMission(state).find((item)=>item.kind==="warmup")).toMatchObject({minutes:4,titleDe:"Abruf-Warm-up"});expect(state.reviewItems).toHaveLength(0)});
