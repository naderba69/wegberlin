...
describe("published academic lesson integrity",()=>{
  ...
  it("builds a balanced 80-item B2 final bank",()=>{
    ...
    expect(bank).toHaveLength(96);
    // Four items from each published lesson of the module; the module size is read from the curriculum, so a new lesson cannot silently drop out of the bank.
    for(let moduleNumber=1;moduleNumber<=6;moduleNumber+=1){const lessons=b2.filter((lesson)=>lesson.module===moduleNumber);expect(lessons.length).toBeGreaterThanOrEqual(2);expect(lessons.flatMap((lesson)=>lesson.miniTest.slice(0,4)), `module ${moduleNumber}`).toHaveLength(lessons.length*4);}
  });
