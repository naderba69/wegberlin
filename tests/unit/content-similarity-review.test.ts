...
describe("internal near-duplicate and independent review-state governance",()=>{
  ...
  it("scans the complete live closed/productive corpus with no unexempt pair",()=>{
    ...
    expect(audit.similarity).toMatchObject({policyVersion:"content-near-duplicate-v1",entryCount:3277,comparedPairCount:5367726,suspiciousPairCount:10,exemptSameContextCount:10,exemptReviewedCount:0,issueCount:0});
  ...
  it("keeps German, Arabic, CEFR, and copyright states separate on every object",()=>{
    const audit=liveAudit;
    expect(audit.reviewState).toMatchObject({policyVersion:"content-review-state-v1",rowCount:3277});
    for(const dimension of ["german","arabic","cefr","copyright"] as const){
      expect(audit.reviewState.reviewCounts[dimension]).toEqual({total:3277,automatedPassHumanPending:3277,independentlyReviewed:0});
  ...
  it("commits synchronized machine/readable reports with explicit copyright limits",()=>{
    ...
    expect(markdown).toContain("5,367,726 deterministic pairs");
