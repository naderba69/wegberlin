...
describe("P0 exhaustive language and Bidi boundaries",()=>{
  ...
  it("audits every TSX opening tag with zero pairing or control-character issues",()=>{
    expect(report).toMatchObject({format:"dwnb-language-boundary-audit",version:"language-boundary-audit-v1",ok:true,tsxFiles:180,openingTagCount:7046,germanTagCount:406,technicalScopeCount:45,adaptiveConsumerCount:10,mixedStaticCount:235,issues:[]});
  ...
  it("commits a readable report with the same content hash and honest manual boundary",()=>{
    ...
    // The committed report must carry the same counts the audit produced. The
    // figure is read from the audit rather than hard-coded, because formatting
    // changes in any TSX file legitimately move it and a stale literal would
    // fail for reasons unrelated to the bidi guarantee.
    expect(report.mixedStaticCount).toBeGreaterThan(200);
    expect(markdown).toContain(String(report.mixedStaticCount));
    expect(markdown).toContain(String(report.germanTagCount));
