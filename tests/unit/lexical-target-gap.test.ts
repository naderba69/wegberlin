...
describe("lexical target-gap inventory", () => {
  it("inventories all lessons and preserves the authored anchor baseline", () => {
    ...
    expect(audit.lessonCount).toBe(84);
    expect(audit.nounAnchorCount).toBe(1044);
    expect(audit.verbFrameAnchorCount).toBe(104);
  ...
  it("commits deterministic v1 candidate and gap counters", () => {
    expect(audit.nounSummary).toMatchObject({
      ...
      covered: 1106,
      pendingHuman: 0,
      contextualNotTarget: 2113,
      authoredTargets: 1106,
    });
    expect(audit.verbFrameSummary).toMatchObject({
      totalCandidates: 817,
      covered: 104,
      pendingHuman: 0,
      contextualNotTarget: 713,
      authoredTargets: 104,
    });
  });
  ...
  it("surfaces real lesson-scoped target decisions instead of claiming four anchors are exhaustive", () => {
    ...
    expect(germanLanguage?.anchorIds).toEqual(["a1-02-noun-8"]);
    ...
    expect(audit.nounSummary.byLevel.B2.pendingHuman).toBe(0);
    ...
    expect(audit.exclusionDecisionCount).toBe(6);
    expect(audit.pendingIndependentExclusionReview).toBe(6);
